...
const MedicationsPage = () => {
    ...
    const [formData, setFormData] = useState({
        ...
        scheduledTimes: [], startDate: '', endDate: '',
    ...
    const handleAddTime = () => {
        if (timeInput && !formData.scheduledTimes.includes(timeInput)) {
            setFormData({ ...formData, scheduledTimes: [...formData.scheduledTimes, timeInput].sort() });
            setTimeInput('');
        }
    };

    const handleRemoveTime = (time) => {
        setFormData({ ...formData, scheduledTimes: formData.scheduledTimes.filter(t => t !== time) });
    };
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        setError(''); setSuccess('');
        if (formData.scheduledTimes.length === 0) { setError('Please add at least one scheduled time'); return; }
        try {
            const medicationData = {
                name: formData.name, dosage: formData.dosage,
                frequency: formData.frequency, scheduledTimes: formData.scheduledTimes,
                instructions: formData.instructions, sideEffects: formData.sideEffects,
            ...
            if (response.success) {
                ...
                setShowAddModal(false); setEditingMed(null); resetForm();
    ...
    const handleEdit = (medication) => {
        ...
        setFormData({
            ...
            instructions: medication.instructions || '', sideEffects: medication.sideEffects || '',
        });
    ...
    const handleDelete = async (medicationId) => {
        ...
        } catch { setError('Failed to delete'); setTimeout(() => setError(''), 5000); }
    };
    ...
    const handleLogTaken = async (medicationId, medicationName) => {
        ...
        } catch { setError('Failed to log'); setTimeout(() => setError(''), 5000); }
    };

    const resetForm = () => {
        setFormData({ name: '', dosage: '', frequency: 'DAILY', scheduledTimes: [], startDate: '', endDate: '', instructions: '', sideEffects: '' });
        setTimeInput('');
    };
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            ...
            <nav className="sticky top-0 z-50 bg-white border-b border-gray-200 shadow-sm">
                <div className="max-w-6xl mx-auto px-4 h-12 flex items-center gap-3">
                    <button onClick={() => navigate('/dashboard')}
                        className="text-gray-400 hover:text-gray-800 font-bold text-lg leading-none">←</button>
                    <span className="font-bold text-gray-900 text-sm">💊 Medications</span>
            ...
            <div className="max-w-6xl mx-auto px-4 py-5">
                ...
                {/* Notifications */}
                {error && <div className="mb-3 p-3 bg-red-50 border border-red-200 rounded-lg text-red-700 text-sm">⚠️ {error}</div>}
                    ...
                    <div className="flex gap-1.5">
                        {['all', 'active', 'inactive'].map(f => (
                            <button key={f} onClick={() => setFilter(f)}
                                className={`px-3 py-1.5 rounded-lg text-xs font-semibold capitalize transition-colors ${filter === f ? 'bg-blue-600 text-white' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50'}`}>
                                {f}
                            </button>
                        ))}
                    </div>
                    <button
                        onClick={() => { setEditingMed(null); resetForm(); setShowAddModal(true); }}
                ...
                {filteredMedications.length === 0 && (
                    <div className="text-center py-16 text-gray-400">
                        ...
                        <p className="font-medium">No {filter !== 'all' ? filter : ''} medications found</p>
                        <button onClick={() => { setEditingMed(null); resetForm(); setShowAddModal(true); }}
                    ...
                    {filteredMedications.map(med => {
                        ...
                        return (
                            <div key={med.id}
                                ...
                                {/* Top color bar */}
                                <div className={`h-1 w-full ${med.active !== false ? 'bg-blue-500' : 'bg-gray-300'}`} />

                                <div className="p-4">
                                    {/* Header row */}
                                    <div className="flex items-start justify-between gap-2 mb-3">
                                        <div className="min-w-0">
                                            <h3 className="font-bold text-gray-900 text-sm truncate">{med.name || 'Unnamed Medicine'}</h3>
                                            {med.dosage && <p className="text-xs text-blue-600 font-medium mt-0.5">{med.dosage}</p>}
                                        </div>
                                        <span className={`text-xs font-bold px-2 py-0.5 rounded-full shrink-0 ${med.active !== false ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                                    ...
                                    <div className="space-y-1.5 text-xs text-gray-600 mb-3">
                                        <div className="flex items-center gap-2">
                                            <span className="text-gray-400 w-16 shrink-0">Frequency</span>
                                            <span className="font-medium">{med.frequency?.replace(/_/g, ' ')}</span>
                                        </div>
                                        {nextDose && (
                                            <div className="flex items-center gap-2">
                                                <span className="text-gray-400 w-16 shrink-0">Next dose</span>
                                                <span className="font-bold text-blue-700 bg-blue-50 px-2 py-0.5 rounded">{nextDose}</span>
                                            </div>
                                        )}
                                        {med.startDate && (
                                            <div className="flex items-center gap-2">
                                                <span className="text-gray-400 w-16 shrink-0">Start</span>
                                                <span>{new Date(med.startDate).toLocaleDateString('en-IN')}</span>
                                            </div>
                                        )}
                                        {med.endDate && (
                                            <div className="flex items-center gap-2">
                                                <span className="text-gray-400 w-16 shrink-0">End</span>
                                                <span className="text-red-600">{new Date(med.endDate).toLocaleDateString('en-IN')}</span>
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    <div className="flex gap-2 pt-3 border-t border-gray-100">
                                        <button onClick={() => handleLogTaken(med.id, med.name)}
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-end sm:items-center justify-center p-0 sm:p-4 z-50">
                    <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full sm:max-w-lg max-h-[92vh] overflow-y-auto">
                        <div className="sticky top-0 bg-white border-b border-gray-100 px-5 py-4 flex items-center justify-between rounded-t-2xl">
                            <h2 className="font-bold text-gray-900">{editingMed ? '✏️ Edit Medication' : '+ Add Medication'}</h2>
                            <button onClick={() => { setShowAddModal(false); setEditingMed(null); resetForm(); }}
                        ...
                        <form onSubmit={handleSubmit} className="p-5 space-y-4">
                            {/* Name */}
                            <div>
                                <label className="block text-xs font-semibold text-gray-600 mb-1">Medication Name *</label>
                            ...
                            {/* Scheduled Times */}
                            <div>
                                <label className="block text-xs font-semibold text-gray-600 mb-1">Scheduled Times *</label>
                                ...
                                {formData.scheduledTimes.length > 0 && (
                                    <div className="flex flex-wrap gap-1.5">
                                        {formData.scheduledTimes.map(t => (
                                            <span key={t} className="flex items-center gap-1 text-xs bg-blue-50 text-blue-700 border border-blue-100 px-2 py-1 rounded-full">
                                                🕐 {t}
                                                <button type="button" onClick={() => handleRemoveTime(t)}
                            ...
                            <div>
                                ...
                                <textarea name="instructions" value={formData.instructions} onChange={handleChange} rows={2}
                            ...
                            {/* Side Effects */}
                            <div>
                                <label className="block text-xs font-semibold text-gray-600 mb-1">Side Effects (optional)</label>
                                <textarea name="sideEffects" value={formData.sideEffects} onChange={handleChange} rows={2}
                                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-blue-400 resize-none"
                            ...
                            <div className="flex gap-3 pt-2">
                                ...
                                <button type="button"
                                    onClick={() => { setShowAddModal(false); setEditingMed(null); resetForm(); }}
